...
class Player(object):
    def __init__(self,name,num_dice=1,strategy=('Random',6)):
    ...
    def Roll(self,targetscore):
        ...
        if (self.TotalScore / targetscore) >= .8:
            self.Strategy = 'Sprint_To_Finish'
        
        if self.Strategy == 'Random':
        	Score,NumRolls = self.RandomRoll(targetscore)
        elif self.Strategy == 'Aggressive':
            Score,NumRolls = self.AggressiveRoll(targetscore)
        elif self.Strategy == 'Cautious':
            Score,NumRolls = self.CautiousRoll(targetscore)
        elif self.Strategy == 'Robust':
            pass
        elif self.Strategy == 'CopyCat':
            pass
        elif self.Strategy == 'Sprint_To_Finish':
            Score,NumRolls = self.SprintToFinish(targetscore)
        
        self.TotalScore += Score
        self.LastScore = Score
        self.LastNumRolls = NumRolls
       
    def SprintToFinish(self,targetscore):
    	Score = 0
    	NumRolls = 0
    	
    	while (self.TotalScore < targetscore):
    		NumRolls += 1
    		roll = self.pig.Roll()
    		if roll == 0:
    			break
    		...
    		if (self.TotalScore >= targetscore):
    			print(self.Name + " has reached " + str(targetscore) + " while Sprinting to the Finish ,making them the winner!")
    			break
    	return (Score,NumRolls)
       
    def CautiousRoll(self, targetscore):
    	Score = 0
    	NumRolls = 0
    	...
    	for i in range(random.randint(3,6)):
    		if (self.TotalScore < targetscore):
    			...
    			self.TotalScore += roll
    		if(self.TotalScore >= targetscore):
    			print(self.Name + " has reached " + str(targetscore) + " with the Safe Strat ,making them the winner!")
    			break
    	return (Score,NumRolls)
    			
    
    def AggressiveRoll(self, targetscore):
    	Score = 0
    	NumRolls = 0
    	...
    	for i in range(random.randint(6,9)):
    		if (self.TotalScore < targetscore):
    			...
    			self.TotalScore += roll
    		if(self.TotalScore >= targetscore):
    			print(self.Name + " has reached " + str(targetscore) + " with Agressive Strat ,making them the winner!")
    			break
    	return (Score,NumRolls)
    ...
    def RandomRoll(self,targetscore):
        ...
        for i in range(random.randint(1,6)):
            if (self.TotalScore < targetscore):
                ...
                self.TotalScore += roll
            if(self.TotalScore >= targetscore):
                print(self.Name + " has reached " + str(targetscore) + " with the Random Strat, making them the winner!")
...
def main():
	...
	for i in range(0, 100):
		...
		p2 = Player('bob', 1, ('Cautious', 3))
		p3 = Player('dax')
		p4 = Player('sue')
		
		AllPlayers = [p1, p2, p3, p4]
		
		kwargs = {'num_dice':1,'random_roles':7,'target_score':100,'players':AllPlayers}
